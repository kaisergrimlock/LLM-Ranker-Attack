...
#
# This reruns only GPT-OSS-20B pointwise DOH/DCH after the pointwise evaluator
# patch that sends reasoning_effort=low for GPT-OSS Bedrock calls. The default
# RUN_ID is intentionally distinct from the old pointwise_doh_dch directory, so
# stale completed checkpoints with empty responses are not reused.
...
N_JOBS="${N_JOBS:-2}"
RUN_ID="${RUN_ID:-gptoss_pointwise_low_reasoning}"
...
# GPT-OSS can spend small output budgets before producing visible label text.
# The evaluator also passes reasoning_effort=low for this model.
