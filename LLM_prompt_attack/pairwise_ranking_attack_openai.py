...
def prepare_pairs(
    ...
    if pos_rel not in actual_levels:
        raise ValueError(
            f"pos_rel={pos_rel} not found in dataset. Available levels: {actual_levels}"
        )
    if neg_rel not in actual_levels:
        raise ValueError(
            f"neg_rel={neg_rel} not found in dataset. Available levels: {actual_levels}"
        )
        ...
        for pos_doc_id, neg_doc_id in sampled:
            try:
                ...
                if pos_doc_data is None or neg_doc_data is None:
                    continue
                pos_doc = Document(
                    pos_doc_id, truncate_text(pos_doc_data.text, model_name), pos_rel
                )
                neg_doc = Document(
                    neg_doc_id, truncate_text(neg_doc_data.text, model_name), neg_rel
                )
    ...
    if not pairs:
        raise ValueError(
            f"No document pairs found with relevance levels pos={pos_rel}, neg={neg_rel}. "
    ...
    if skipped_queries > 0:
        ...
        print(
            f"Note: {skipped_queries}/{total_queries} queries skipped (missing docs at pos={pos_rel} or neg={neg_rel})"
        )
...
def _prepare_pairs_with_negative_sampling(
    dataset,
    docstore,
    queries,
    qrels_df,
        ...
        for pos_doc_id in pos_doc_ids[:10]:  # Limit positives per query
            for neg_doc_id in neg_doc_ids:
                try:
                    ...
                    pos_doc = Document(
                        pos_doc_id,
                        truncate_text(pos_doc_data.text, model_name),
                        pos_rel,
                    )
                    neg_doc = Document(
                        neg_doc_id, truncate_text(neg_doc_data.text, model_name), 0
                    )  # 0 = sampled negative
...
def get_choices_openai(
    ...
    def worker(pair):
        query, doc1, doc2 = pair
        return _process_single_query_pairwise(
            query,
            doc1,
            doc2,
            model_name,
    ...
    if checkpoint is not None:
        ...
        results = Parallel(n_jobs=n_jobs, backend="threading")(
            delayed(worker)(pair) for pair in tqdm(pairs, desc=f"Querying {provider}")
        )
...
def apply_attack(results, pairs, attack_prompt: str, attack_position: str = "back"):
    ...
    for i, result in enumerate(results):
        ...
        target = doc2 if result == "A" else doc1
        attacked_doc = Document(
            target.doc_id,
            render_attack_text(attack_prompt, query, target.text, attack_position),
            target.relevance,
        )
        if result == "A":
...
def main():
    ...
    if isinstance(attack_payload, KeywordInjection):
        attack_payload.validate_queries(pairs)
    ...
    attacked_pairs = apply_attack(
        valid_rankings, valid_pairs, attack_payload, args.attack_position
    )
